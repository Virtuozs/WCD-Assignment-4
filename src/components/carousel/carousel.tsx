...
const ImageCarousel: React.FC<ImageCarouselProps> = ({
  ...
  return (
    <div className="w-screen overflow-hidden select-none my-10">
      <div
        ref={containerRef}
        className="flex flex-row gap-10 whitespace-nowrap will-change-transform leading-none"
        style={{
        ...
        {[...Array(2)].map((_, i) => (
          <div className="carousel-set flex flex-row gap-10" key={i}>
            {items.map((item, index) => (
                <div
                    key={`${i}-${index}`}
                    className="group relative flex min-w-[400px] h-64 bg-[#C7D0D9] py-14 px-12 flex-shrink-0 rounded-sm"
                    >
                    <img
                        src={item.image}
